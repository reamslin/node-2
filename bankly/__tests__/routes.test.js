...
describe("POST /auth/login", function () {
  ...
  //TESTS bug #6
  test("should return 401 for incorrect username/password", async function () {
    const response = await request(app)
      .post("/auth/login")
      .send({
        username: "u1",
...
// should deny access if invalid token
//TESTS bug #3
describe("GET /users", function () {
  test("should deny access with invalid token", async function () {
    const response = await request(app)
      .get("/users")
      .send({ _token: "not a token" });
    expect(response.statusCode).toBe(401);
    console.log("error message is: ", response.body);
  })
})
describe("GET /users", function () {
...
describe("GET /users/[username]", function () {
  ...
  //TESTS bug #4
  test("should give 404 if no such user", async function () {
    const response = await request(app)
      .get("/users/nope")
...
describe("PATCH /users/[username]", function () {
  ...
  test("should return 404 for no such user", async function () {
    const response = await request(app)
      .patch("/users/nope")
      .send({ _token: tokens.u3, first_name: "new" });
    expect(response.statusCode).toBe(404);
  });

  //need to test that user can update own data 
  ...
  //TESTS bug #1
  test("user should be allowed to update own data", async function () {
    const response = await request(app)
      .patch("/users/u1")
      .send({ _token: tokens.u1, first_name: "new" });
    expect(response.statusCode).toBe(200);
    expect(response.body.user).toEqual({
      username: "u1",
      first_name: "new",
